...
async function draw(opts = {}) {
  ...
  const semW = (attuned && q && !opts.picks && window.ARCANAI_ATTUNE)
    ? await ARCANAI_ATTUNE.weightsFor(q, 450) : null;
  const themes = (!semW && attuned && q && !opts.picks) ? questionThemes(q) : new Set();
  const leaned = !!semW || themes.size > 0;
  const picks = opts.picks ||
    (semW ? weightedOrder(rng, semW) : themes.size ? attunedOrder(rng, themes) : shuffled(rng))
      .slice(0, mode).map((c, i) => ({ ...c, rev: rng() < .5, label: LABELS[mode][i] || '' }));
...
with open(os.path.join(REL, 'index.html'), 'w') as f:
    f.write(html)
# the semantic shuffle rides along as a static asset; source of truth is cards/
