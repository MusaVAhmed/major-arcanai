...
<style>
  ...
  * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
  ...
  /* masthead: styled as one of the deck's own card frames —
     double-rule border, cartouche eyebrow, banner word, star ornaments */
  .mast {
    position:relative; margin:0 auto 1.5rem; max-width:24rem;
    border:1px solid var(--line); padding:.9rem .9rem .8rem;
  }
  .mast::before { content:''; position:absolute; inset:4px; border:1px solid var(--line); pointer-events:none; }
  .mast .star { position:absolute; top:-.62em; left:50%; transform:translateX(-50%);
                background:var(--ink); padding:0 .55em; color:var(--dim); font-size:.85rem;
                line-height:1; user-select:none; }
  h1 { margin:0; font-family:'Cinzel', Georgia, serif; text-transform:uppercase; line-height:1; }
  h1 .eyebrow {
    display:inline-block; font-size:.62rem; font-weight:600; letter-spacing:.3em; color:var(--dim);
    border:1px solid var(--line); border-radius:999px;
    padding:.42em .85em .42em calc(.85em + .3em); margin-bottom:.6rem;
  }
  h1 .word {
