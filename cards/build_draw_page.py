...
/* whether the spread on the table was drawn with the question still in the box */
let SPENT = false;
...
/* The deck used to keep a log of the cards it had shown you so it could remark
   when one kept coming back. It was removed: a reading is about the cards on
   the table, and being told the deck has been counting reads as nagging rather
   than as noticing. Sweep the key so the old log does not sit in localStorage
   forever on devices that used it. */
try { localStorage.removeItem('arcanai-cardlog'); } catch (_) {}
...
for (const n of [1,3,5,10]) {
  document.getElementById('m'+n).addEventListener('click', () => {
    ...
    closeGallery();
    /* An explicit spread-type switch arms a fresh deal; only browser reloads
       and tab switches preserve the reading. Whether it also wipes the question
       turns on whether that question has been spent. Typed one and changed your
       mind about One versus Cross before drawing? Keep it — losing what you
       typed for that is infuriating. Already drew with it? Wipe it. `.ask` is
       hidden for the whole reading, so a question that survives its own spread
       is one you cannot see, will not remember typing, and will find silently
       attached to the next reading — which reads as the deck asking on your
       behalf. */
    idle(SPENT);
  });
}
...
/* `wipe` means a reading actually ended, and the question ends with it: asking
   ...
   thing tarot is genuinely strict about. Switching spread type passes `SPENT`
   rather than a constant — see the mode buttons. */
function idle(wipe) {
...
async function draw(opts = {}) {
  ...
  /* Whether the spread now on the table was drawn with the question that is
     still sitting in the box. `.ask` is hidden for the whole reading, so a
     question that outlives its spread is one you cannot see and will not
     remember typing — see the spread-type switch in the mode buttons. */
  SPENT = !!q;
  CUR = picks;
...
/* ---- the view swap: the whole spread at once, or one card at a time ----
   ...
   which of them are turned over, and any clarifiers — so
...
function swapView() {
  ...
  let st = null;
  try { st = JSON.parse(sessionStorage.getItem(SKEY)); } catch (_) {}
  draw({ picks: CUR, restore: st || { revealed: [] } });
}
...
function showVerdict() {
  const res = resonance();
  const line = (res && resonantVerdict(res)) || tagVerdict();
  if (!line) return;
